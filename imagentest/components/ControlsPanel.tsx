...
const ControlsPanel: React.FC<ControlsPanelProps> = ({ appState, setAppState, availableFonts, onDownloadImage }) => {
  ...
  const handleBgImageUpload = (image: string | null) => {
    setAppState(prev => ({ ...prev, backgroundImage: image, imageGenerationError: null }));
  };
  ...
  const handleDefaultImageGenerationPrefixPromptChange = (prompt: string) => {
    setAppState(prev => ({ ...prev, defaultImageGenerationPrefixPrompt: prompt }));
  };
  
  const handleImagenApiKeyChange = (apiKey: string) => {
    setAppState(prev => ({ ...prev, imagenApiKey: apiKey }));
  };

  const handleGenerateImage = async () => {
    const userEnteredPrompt = appState.imageGenerationPrompt.trim();
    const prefix = appState.defaultImageGenerationPrefixPrompt.trim();
    let combinedPrompt = "";

    if (prefix) {
      combinedPrompt = prefix;
      if (userEnteredPrompt) {
        if (!prefix.endsWith(',') && !prefix.endsWith(' ')) {
          combinedPrompt += ", ";
        } else if (prefix.endsWith(',') && !prefix.endsWith(' ')) { // Ends with comma but not space
           combinedPrompt += " ";
        }
        combinedPrompt += userEnteredPrompt;
      }
    } else {
      combinedPrompt = userEnteredPrompt;
    }
    ...
    if (!combinedPrompt.trim()) {
      setAppState(prev => ({ ...prev, imageGenerationError: "Prompt cannot be empty." }));
      return;
    }
    ...
    try {
      if (!appState.imagenApiKey) {
        throw new Error("API key is not provided.");
      }
      const ai = new GoogleGenAI({ apiKey: appState.imagenApiKey });
      ...
      if (response.generatedImages && response.generatedImages.length > 0 && response.generatedImages[0].image?.imageBytes) {
        ...
        setAppState(prev => ({ ...prev, backgroundImage: imageUrl, backgroundImageDimness: 0, imageGenerationError: null }));
      } else {
      ...
      if (error.message && error.message.includes('API key')) {
         setAppState(prev => ({ ...prev, imageGenerationError: "Please enter a valid API Key in the field above."}));
      } else {
  ...
  return (
    <div className="w-full p-4 md:p-6 space-y-6 overflow-y-auto bg-stone-50 rounded-xl shadow-2xl print:hidden">
      ...
      <BackgroundControls
        ...
        defaultImageGenerationPrefixPrompt={appState.defaultImageGenerationPrefixPrompt}
        ...
        onDefaultImageGenerationPrefixPromptChange={handleDefaultImageGenerationPrefixPromptChange}
        onImageGenerationPromptChange={handleImageGenerationPromptChange}
        onGenerateImage={handleGenerateImage}
        imagenApiKey={appState.imagenApiKey}
        onImagenApiKeyChange={handleImagenApiKeyChange}
      />
